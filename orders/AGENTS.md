# Orders Service Documentation Instructions

## Scope

- Orders service source is referenced as `@orders`.
...
- When service behavior depends on another service, event, database, or backend contract, inspect the relevant local source or configuration. Do not guess.

## README Rules

- The only permitted file edit for this documentation task is `@orders/README.md`.
- Read any existing `@orders/README.md` before editing it.
- Preserve content that is accurate and still useful.
- Verify claims from source, tests, package scripts, Dockerfile, TypeScript configuration, database models, and relevant infrastructure configuration.
...
- Use `@orders`-style paths in README documentation where practical.
...
- Main order workflows and lifecycle states.
...
- Database models, persistence behavior, and concurrency/versioning behavior when present.
