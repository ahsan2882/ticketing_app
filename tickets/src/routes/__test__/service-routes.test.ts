...
describe("tickets service health and fallback routes", () => {
  beforeEach(() => {
  ...
  it("returns 200 only when both MongoDB and NATS are ready", async () => {
    healthState.setReady("mongo");
  ...
  it("returns 404 for an unknown route", async () => {
    await request(app).get("/api/tickets-does-not-exist").expect(404);
  });
});
